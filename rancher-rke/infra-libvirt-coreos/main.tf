...
resource "libvirt_volume" "additional_disks2" {
  name  = "disk2_${count.index}.qcow2"
  size  = "${var.additional_disks_size}"
  count = "${var.qtd_vms}"
}

resource "libvirt_volume" "additional_disks3" {
  name  = "disk3_${count.index}.qcow2"
  size  = "${var.additional_disks_size}"
  count = "${var.qtd_vms}"
}
...
resource "libvirt_domain" "my_machine" {
  ...
  disk {
    volume_id = "${libvirt_volume.additional_disks2.*.id[count.index]}"
  }

  disk {
    volume_id = "${libvirt_volume.additional_disks3.*.id[count.index]}"
  }
}
