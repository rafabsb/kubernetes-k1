...
resource "libvirt_domain" "my_machine" {
  ...
  network_interface {
    ...
    network_name = "net1"
    mac          = "${element(var.vm_macs, count.index)}"
  ...
  # depends_on = ["libvirt_volume.main_disks", "libvirt_cloudinit.commoninit"]
}

# output "ip" {
#   value      = "${libvirt_domain.my_machine.network_interface.0.addresses.0}"
#   depends_on = ["libvirt_domain.my_machine"]
# }
